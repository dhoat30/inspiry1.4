 ...
 */ 
  ...
  //error_log( print_r(  '$subtotal before the IF = ' .$subtotal, true ) );
      ...
      //v2.0.2.02 c - begin  ELSE STRUCTURE
     // crossout when session variable not stored
        else { 
        
      //error_log( print_r(  '$subtotal after ELSE = ' .$subtotal, true ) );
        
        $oldprice_subtotal = wc_price( $_SESSION['vtprd_orig_cart_price_'.$cart_item_key] * $cart_item['quantity'] );
        
        $_SESSION['vtprd_oldprice_subtotal_with_taxation_'.$product_id] = $oldprice_subtotal; //even though this may not have taxation, subtotal is stored here to be picked up later in existing logic.
     ...
     $this->vtprd_store_session_transient_with_ip_key();
  ...
  //**************************************
    ...
    */      
  //**************************************
  public function vtprd_store_session_transient_with_ip_key(){
    global $vtprd_info;
  ...
  public function vtprd_maybe_order_formatted_line_subtotal($subtotal, $item, $data){
   ...
   //error_log( var_export($vtprd_info, true ) );     
    ...
    //get session data stored under IP address in function vtprd_store_session_transient_with_ip_key 
    // use IP instead of cart_id as unique identifier
    $serialized_session = vtprd_get_transient_cart_data ( 'session_data_by_ip', $vtprd_info['purchaser_ip_address']);
    if ($serialized_session) { 
        $_SESSION = unserialize($serialized_session);
    }     
    //v2.0.2.02 END
    ...
    if ( (isset ($_SESSION['do_no_actions'])) &&
     ...
     /*
     //v2.0.2.02 e BEGIN
     // Session unique_cart_id gets lost by now, as of WOO 5.0
    ...
    if ($item['line_subtotal'] == 0) { 
      ...
      //v2.0.2.0 end         
        ...
        if ( (isset($previous_auto_add_array[$product_id])) &&  //v2.0.2.02
             (apply_filters('vtprd_show_zero_price_as_free',TRUE)) ) { //if zero is preferred, send back FALSE
           session_write_close(); //v2.0.2.02
    ...
    */
    ...
    //v1.1.0.6 end
     //error_log( print_r(  'Getting the Oldprice Subtotal, for $product_id= ' .$product_id, true ) );
        ...
        //error_log( print_r(  'vtprd_maybe_order_formatted_line_subtotal - $_SESSION', true ) );
