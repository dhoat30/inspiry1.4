  ...
  //error_log( var_export($vtprd_rules_set[$i], true ) ); 
   ...
   public function vtprd_manage_shared_rule_tests($i) { 
    ...
    //error_log( print_r(  'vtprd_manage_shared_rule_tests ', true ) ); 
      ...
      //v2.0.0 added if ( (defined('VTPRD_PRO_VERSION'))
      if ( (defined('VTPRD_PRO_VERSION')) &&  
  ...
  // ****************     
   ...
   public function vtprd_manage_customer_rule_tests($i) { 
     ...
     if ( ($vtprd_rules_set[$i]->buy_group_population_info['buy_group_customer_set_to_include_exclude_both_none'] == 'excludeOnly') ||
          ($vtprd_rules_set[$i]->buy_group_population_info['buy_group_customer_set_to_include_exclude_both_none'] == 'both') ) {
             
          
         //customer exclusions
         //exclusions take precedence over ANY other criteria.
         if (sizeof($vtprd_rules_set[$i]->buy_group_population_info['buy_group_role_excl_array']) > 0) {
            if ($this->vtprd_is_role_in_inPop_list_check($i, $k, 'excl', $vtprd_rules_set[$i]->buy_group_population_info['buy_group_role_excl_array']) ) {
              $vtprd_rules_set[$i]->rule_status = 'custInvalid';  
              $vtprd_rules_set[$i]->rule_processing_status = 'Role exclusion found';
              $vtprd_cart->cart_items[$k]->cartAuditTrail[$vtprd_rules_set[$i]->post_id]['inPop_participation_msgs'][] = 'Role exclusion found - Customer + Rule Invalid'; 
     //error_log( print_r(  'vtprd_is_product_in_inPop_group return false 007 ', true ) );
              return;
            } else {
              $vtprd_cart->cart_items[$k]->cartAuditTrail[$vtprd_rules_set[$i]->post_id]['inPop_participation_msgs'][] = 'Customer Roles list did NOT match exclusion list'; 
              $cust_exclusion_listed_in_rule_but_not_matched = true;       
            }
         }
         if (sizeof($vtprd_rules_set[$i]->buy_group_population_info['buy_group_email_excl_array']) > 0) {
            if ($this->vtprd_is_email_in_inPop_list_check($i, $k, 'excl', $vtprd_rules_set[$i]->buy_group_population_info['buy_group_email_excl_array']) ) {
              $vtprd_rules_set[$i]->rule_status = 'custInvalid';  
              $vtprd_rules_set[$i]->rule_processing_status = 'Customer Email/Name exclusion found';
              $vtprd_cart->cart_items[$k]->cartAuditTrail[$vtprd_rules_set[$i]->post_id]['inPop_participation_msgs'][] = 'Customer Email/Name exclusion found - Customer + Rule Invalid'; 
     //error_log( print_r(  'vtprd_is_product_in_inPop_group return false 008 ', true ) );
              return;
            } else {
              $vtprd_cart->cart_items[$k]->cartAuditTrail[$vtprd_rules_set[$i]->post_id]['inPop_participation_msgs'][] = 'Customer Email/Name list did NOT match exclusion list'; 
              $cust_exclusion_listed_in_rule_but_not_matched = true;       
            }
         }
     ...
     */
     ...
     //v2.0.2.0 wrap in general test
     if ( ($vtprd_rules_set[$i]->buy_group_population_info['buy_group_customer_set_to_include_exclude_both_none'] == 'includeOnly') ||
          ($vtprd_rules_set[$i]->buy_group_population_info['buy_group_customer_set_to_include_exclude_both_none'] == 'both') ) {
    
         //first test role
         if (sizeof($vtprd_rules_set[$i]->buy_group_population_info['buy_group_role_incl_array']) > 0) {
            if ($this->vtprd_is_role_in_inPop_list_check($i, $k, 'incl', $vtprd_rules_set[$i]->buy_group_population_info['buy_group_role_incl_array']) ) {
              $vtprd_cart->cart_items[$k]->cartAuditTrail[$vtprd_rules_set[$i]->post_id]['inPop_participation_msgs'][] = 'Role inclusion found';
              $vtprd_rules_set[$i]->buy_group_population_info['buy_group_customer_found']  = true;
              return;          
            } else {
              $vtprd_cart->cart_items[$k]->cartAuditTrail[$vtprd_rules_set[$i]->post_id]['inPop_participation_msgs'][] = 'Listed inclusion Role list NOT found for customer';
              ...
              if ($vtprd_rules_set[$i]->buy_group_population_info['buy_group_customer_and_or'] == 'and') {           
                $vtprd_cart->cart_items[$k]->cartAuditTrail[$vtprd_rules_set[$i]->post_id]['inPop_participation_msgs'][] = 'NO Role inclusion found, "AND" on rule';
              }        
            }
         }
         //if customer not found, test email/name     
         if (!$vtprd_rules_set[$i]->buy_group_population_info['buy_group_customer_found']) {
           if (sizeof($vtprd_rules_set[$i]->buy_group_population_info['buy_group_email_incl_array']) > 0) {
              if ($this->vtprd_is_email_in_inPop_list_check($i, $k, 'incl', $vtprd_rules_set[$i]->buy_group_population_info['buy_group_email_incl_array']) ) {
                $vtprd_cart->cart_items[$k]->cartAuditTrail[$vtprd_rules_set[$i]->post_id]['inPop_participation_msgs'][] = 'Customer Email/Name inclusion found';
                $vtprd_rules_set[$i]->buy_group_population_info['buy_group_customer_found']  = true;
                return;    
              } else {
                $vtprd_cart->cart_items[$k]->cartAuditTrail[$vtprd_rules_set[$i]->post_id]['inPop_participation_msgs'][] = 'Listed inclusion Email/Name list NOT found for customer'; 
                $customer_inclusion_criteria_present_but_not_found = true;             
                if ($vtprd_rules_set[$i]->buy_group_population_info['buy_group_customer_and_or'] == 'and') {
                  $vtprd_cart->cart_items[$k]->cartAuditTrail[$vtprd_rules_set[$i]->post_id]['inPop_participation_msgs'][] = 'NO Customer email inclusion found, "AND" on rule';
                }        
              }
           }
         }
   ...
   public function vtprd_is_product_in_inPop_group($i, $k) { 
     ...
     */
     ...
     if ( ($vtprd_rules_set[$i]->buy_group_population_info['buy_group_products_set_to_include_exclude_both_none'] == 'excludeOnly') ||
         ...
         //product, cat etc exclusions
         if (sizeof($vtprd_rules_set[$i]->buy_group_population_info['buy_group_prod_cat_excl_array']) > 0) {
            if ( array_intersect($vtprd_rules_set[$i]->buy_group_population_info['buy_group_prod_cat_excl_array'], $vtprd_cart->cart_items[$k]->prod_cat_list ) ) {
              $vtprd_cart->cart_items[$k]->cartAuditTrail[$vtprd_rules_set[$i]->post_id]['inPop_participation_msgs'][] = 'ProdCat exclusion found';
     //error_log( print_r(  'vtprd_is_product_in_inPop_group return false 001 ', true ) );         
              return false;
            }
         }
    
         if (sizeof($vtprd_rules_set[$i]->buy_group_population_info['buy_group_plugin_cat_excl_array']) > 0) {
            if ( array_intersect($vtprd_rules_set[$i]->buy_group_population_info['buy_group_plugin_cat_excl_array'], $vtprd_cart->cart_items[$k]->rule_cat_list ) ) {
              $vtprd_cart->cart_items[$k]->cartAuditTrail[$vtprd_rules_set[$i]->post_id]['inPop_participation_msgs'][] = 'PluginCat exclusion found';
     //error_log( print_r(  'vtprd_is_product_in_inPop_group return false 002 ', true ) );
              return false;
            }
         }
         
         if (sizeof($vtprd_rules_set[$i]->buy_group_population_info['buy_group_product_excl_array']) > 0) {
            if ( in_array($vtprd_cart->cart_items[$k]->product_id, $vtprd_rules_set[$i]->buy_group_population_info['buy_group_product_excl_array'] ))  {
              $vtprd_cart->cart_items[$k]->cartAuditTrail[$vtprd_rules_set[$i]->post_id]['inPop_participation_msgs'][] = 'Product exclusion found';
     //error_log( print_r(  'vtprd_is_product_in_inPop_group return false 003 ', true ) );
              return false;
            }
            if ($vtprd_cart->cart_items[$k]->parent_product_id) { //if this is a variation product, check for parent!
              if ( in_array($vtprd_cart->cart_items[$k]->parent_product_id, $vtprd_rules_set[$i]->buy_group_population_info['buy_group_product_excl_array'] ))  {
                $vtprd_cart->cart_items[$k]->cartAuditTrail[$vtprd_rules_set[$i]->post_id]['inPop_participation_msgs'][] = 'Product exclusion found';
     //error_log( print_r(  'vtprd_is_product_in_inPop_group return false 004 ', true ) );
                return false;
              }        
            }
         }
     ...
     */
     ...
     //product, cat etc inclusions
     if (sizeof($vtprd_rules_set[$i]->buy_group_population_info['buy_group_prod_cat_incl_array']) > 0) {
        if ( array_intersect($vtprd_rules_set[$i]->buy_group_population_info['buy_group_prod_cat_incl_array'], $vtprd_cart->cart_items[$k]->prod_cat_list ) ) {
           $vtprd_cart->cart_items[$k]->cartAuditTrail[$vtprd_rules_set[$i]->post_id]['inPop_participation_msgs'][] = 'ProdCat inclusion found';           
           if ($vtprd_rules_set[$i]->buy_group_population_info['buy_group_prod_cat_and_or'] != 'or') { //covers 'and' + 'each'
              $and_tracking++;
           }
           ...
           if ($vtprd_rules_set[$i]->buy_group_population_info['buy_group_prod_cat_and_or'] != 'or') { //covers 'and' + 'each'
              $vtprd_cart->cart_items[$k]->cartAuditTrail[$vtprd_rules_set[$i]->post_id]['inPop_participation_msgs'][] = 'Reguired ProdCat Not found';
 //error_log( print_r(  'vtprd_is_product_in_inPop_group return false 012 ', true ) );
              return false;
           }        
        }
     } else {
           if ($vtprd_rules_set[$i]->buy_group_population_info['buy_group_prod_cat_and_or'] != 'or') { //covers 'and' + 'each'
              $vtprd_cart->cart_items[$k]->cartAuditTrail[$vtprd_rules_set[$i]->post_id]['inPop_participation_msgs'][] = 'Reguired ProdCat Not found';
 //error_log( print_r(  'vtprd_is_product_in_inPop_group return false 013 ', true ) );
              return false;
           } 
     }
     ...
     if (sizeof($vtprd_rules_set[$i]->buy_group_population_info['buy_group_plugin_cat_incl_array']) > 0) {
        if ( array_intersect($vtprd_rules_set[$i]->buy_group_population_info['buy_group_plugin_cat_incl_array'], $vtprd_cart->cart_items[$k]->rule_cat_list ) ) {
          $vtprd_cart->cart_items[$k]->cartAuditTrail[$vtprd_rules_set[$i]->post_id]['inPop_participation_msgs'][] = 'PluginCat inclusion found';
           if ($vtprd_rules_set[$i]->buy_group_population_info['buy_group_plugin_cat_and_or'] != 'or') { //covers 'and' + 'each'
              $and_tracking++;
           }
           if ($vtprd_rules_set[$i]->buy_group_population_info['buy_group_and_switch_count'] == $and_tracking) {
              return true;
           ...
           if ($vtprd_rules_set[$i]->buy_group_population_info['buy_group_plugin_cat_and_or'] != 'or') { //covers 'and' + 'each'
              $vtprd_cart->cart_items[$k]->cartAuditTrail[$vtprd_rules_set[$i]->post_id]['inPop_participation_msgs'][] = 'Reguired PluginCat Not found';
 //error_log( print_r(  'vtprd_is_product_in_inPop_group return false 014 ', true ) );
              return false;
           }        
        }
     } else {
           if ($vtprd_rules_set[$i]->buy_group_population_info['buy_group_plugin_cat_and_or'] != 'or') { //covers 'and' + 'each'
              $vtprd_cart->cart_items[$k]->cartAuditTrail[$vtprd_rules_set[$i]->post_id]['inPop_participation_msgs'][] = 'Reguired PluginCat Not found';
 //error_log( print_r(  'vtprd_is_product_in_inPop_group return false 015 ', true ) );
              return false;
           }      
     }
     
     
     $vtprd_cart->cart_items[$k]->parent_product_id_found_in_search = FALSE;
     
     if (sizeof($vtprd_rules_set[$i]->buy_group_population_info['buy_group_product_incl_array']) > 0) {
        $product_found = false;
        switch (TRUE) {
          case (in_array($vtprd_cart->cart_items[$k]->product_id, $vtprd_rules_set[$i]->buy_group_population_info['buy_group_product_incl_array'])):
              $product_found = true;
            break;
          case (($vtprd_cart->cart_items[$k]->parent_product_id) &&  //if this is a variation product, check for parent!
                (in_array($vtprd_cart->cart_items[$k]->parent_product_id, $vtprd_rules_set[$i]->buy_group_population_info['buy_group_product_incl_array']))):               
              $product_found = true;                
              $vtprd_cart->cart_items[$k]->parent_product_id_found_in_search = true; //mark as having found the PARENT
            break;        
        }
        if ($product_found) {              
          $vtprd_cart->cart_items[$k]->cartAuditTrail[$vtprd_rules_set[$i]->post_id]['inPop_participation_msgs'][] = 'Product inclusion found';
           if ($vtprd_rules_set[$i]->buy_group_population_info['buy_group_product_and_or'] != 'or') { //covers 'and' + 'each'
              $and_tracking++;
           }
           if ($vtprd_rules_set[$i]->buy_group_population_info['buy_group_and_switch_count'] == $and_tracking) {
              return true;
           ...
           if ($vtprd_rules_set[$i]->buy_group_population_info['buy_group_product_and_or'] != 'or') { //covers 'and' + 'each'
              $vtprd_cart->cart_items[$k]->cartAuditTrail[$vtprd_rules_set[$i]->post_id]['inPop_participation_msgs'][] = 'Reguired Product Not found';
 //error_log( print_r(  'vtprd_is_product_in_inPop_group return false 016 ', true ) );
              return false;
           }        
        }
     } else {
           if ($vtprd_rules_set[$i]->buy_group_population_info['buy_group_product_and_or'] != 'or') { //covers 'and' + 'each'
              $vtprd_cart->cart_items[$k]->cartAuditTrail[$vtprd_rules_set[$i]->post_id]['inPop_participation_msgs'][] = 'Reguired Product Not found';
 //error_log( print_r(  'vtprd_is_product_in_inPop_group return false 017 ', true ) );
              return false;
           }     
     }
 ...
 //error_log( print_r(  'vtprd_is_product_in_inPop_group return false 022 ', true ) );
    ...
    public function vtprd_is_role_in_inPop_list_check($i,$k, $incl_or_excl, $test_this_array) {
     ...
     //error_log( print_r(  'vtprd_is_role_in_inPop_list_check ', true ) ); 
      ...
      if (array_intersect($this->vtprd_get_current_user_roles(), $test_this_array )) { //v1.1.7.2 grpC
        if ( $vtprd_setup_options['debugging_mode_on'] == 'yes' ){ 
        ...
        $vtprd_cart->cart_items[$k]->cartAuditTrail[$vtprd_rules_set[$i]->post_id]['inPop_participation_msgs'][] = 'Role found for '.$incl_or_excl ;   //v2.0.1.0a 
        return true;                                
      } else {
        $vtprd_cart->cart_items[$k]->cartAuditTrail[$vtprd_rules_set[$i]->post_id]['inPop_participation_msgs'][] = 'Role Not in checked role list for '.$incl_or_excl ;
        return false; //v2.0.1.0  ADDED - was missing!!        
      } 

      /*  //v2.0.1.0 REMOVED
      $vtprd_cart->cart_items[$k]->cartAuditTrail[$vtprd_rules_set[$i]->post_id]['inPop_role_found'] = 'Role not found for '.$incl_or_excl;
      return false;
      */
    }
    ...
    public function vtprd_is_email_in_inPop_list_check($i,$k, $incl_or_excl, $test_this_array) {
    	global $current_user, $vtprd_rules_set, $vtprd_rule, $vtprd_info, $vtprd_setup_options;           
     //error_log( print_r(  'vtprd_is_email_in_inPop_list_check ', true ) ); 

      //v2.0.1.0a begin
      // changed to use the logged-in test before getting current user
      /*
      AT The wp-admin Rule screen selector "Email or Customer Name",
      the screen does its lookup in the USERS table.
      WHile email OR name can be used in the lookup,
      it may apply ONLY to logged-in users in the front end!!!
      ***  the array stores the USER_ID ONLY ***
      from the lookup performed at rule store time.
      
      Thus we test for User ID and **NOT** Email address or name, even though that's the representation. 
      */
      if( is_user_logged_in() ) {      
        $current_user = wp_get_current_user();     
        if (in_array($current_user->ID, $test_this_array)) {
          $vtprd_cart->cart_items[$k]->cartAuditTrail[$vtprd_rules_set[$i]->post_id]['inPop_participation_msgs'][] = 'User found for '.$incl_or_excl ;     //v2.0.1.0a
          return true;       
        }
      }
      ...
      $vtprd_cart->cart_items[$k]->cartAuditTrail[$vtprd_rules_set[$i]->post_id]['inPop_participation_msgs'][] = 'User not found for  '.$incl_or_excl;   //v2.0.1.0a
      return false;
    }
     ...
     */
     ...
     if ( ($vtprd_rules_set[$i]->action_group_population_info['action_group_products_set_to_include_exclude_both_none'] == 'excludeOnly') ||
         ...
         if (sizeof($vtprd_rules_set[$i]->action_group_population_info['action_group_prod_cat_excl_array']) > 0) {
            if ( array_intersect($vtprd_rules_set[$i]->action_group_population_info['action_group_prod_cat_excl_array'], $vtprd_cart->cart_items[$k]->prod_cat_list ) ) {
              $vtprd_cart->cart_items[$k]->cartAuditTrail[$vtprd_rules_set[$i]->post_id]['actionPop_participation_msgs'][] = 'ProdCat exclusion found';
              return false;
            }
         }
    
         if (sizeof($vtprd_rules_set[$i]->action_group_population_info['action_group_plugin_cat_excl_array']) > 0) {
            if ( array_intersect($vtprd_rules_set[$i]->action_group_population_info['action_group_plugin_cat_excl_array'], $vtprd_cart->cart_items[$k]->rule_cat_list ) ) {
              $vtprd_cart->cart_items[$k]->cartAuditTrail[$vtprd_rules_set[$i]->post_id]['actionPop_participation_msgs'][] = 'PluginCat exclusion found';
              return false;
            }
         }
         
         if (sizeof($vtprd_rules_set[$i]->action_group_population_info['action_group_product_excl_array']) > 0) {
            if ( in_array($vtprd_cart->cart_items[$k]->product_id, $vtprd_rules_set[$i]->action_group_population_info['action_group_product_excl_array'] ))  {
              $vtprd_cart->cart_items[$k]->cartAuditTrail[$vtprd_rules_set[$i]->post_id]['actionPop_participation_msgs'][] = 'Product exclusion found';
              return false;
            }
            if ($vtprd_cart->cart_items[$k]->parent_product_id) { //if this is a variation product, check for parent!
              if ( in_array($vtprd_cart->cart_items[$k]->parent_product_id, $vtprd_rules_set[$i]->action_group_population_info['action_group_product_excl_array'] ))  {
                $vtprd_cart->cart_items[$k]->cartAuditTrail[$vtprd_rules_set[$i]->post_id]['actionPop_participation_msgs'][] = 'Product exclusion found';
                return false;
              }        
            }
     ...
     if ( ($vtprd_rules_set[$i]->action_group_population_info['action_group_products_set_to_include_exclude_both_none'] == 'includeOnly') ||   
         ...
         */
         ...
         if (sizeof($vtprd_rules_set[$i]->action_group_population_info['action_group_prod_cat_incl_array']) > 0) {
            if ( array_intersect($vtprd_rules_set[$i]->action_group_population_info['action_group_prod_cat_incl_array'], $vtprd_cart->cart_items[$k]->prod_cat_list ) ) {
              $vtprd_cart->cart_items[$k]->cartAuditTrail[$vtprd_rules_set[$i]->post_id]['actionPop_participation_msgs'][] = 'ProdCat inclusion found';
                if ($vtprd_rules_set[$i]->action_group_population_info['action_group_prod_cat_and_or'] == 'and') {
                  $and_tracking++;
               }
         ...
         if (sizeof($vtprd_rules_set[$i]->action_group_population_info['action_group_plugin_cat_incl_array']) > 0) {
            if ( array_intersect($vtprd_rules_set[$i]->action_group_population_info['action_group_plugin_cat_incl_array'], $vtprd_cart->cart_items[$k]->rule_cat_list ) ) {
              $vtprd_cart->cart_items[$k]->cartAuditTrail[$vtprd_rules_set[$i]->post_id]['actionPop_participation_msgs'][] = 'PluginCat inclusion found';
               if ($vtprd_rules_set[$i]->action_group_population_info['action_group_plugin_cat_and_or'] == 'and') {
                  $and_tracking++;
               }
               if ($vtprd_rules_set[$i]->action_group_population_info['action_group_and_switch_count'] == $and_tracking) {
                  return true;
         ...
         if (sizeof($vtprd_rules_set[$i]->action_group_population_info['action_group_product_incl_array']) > 0) {
            if (  (in_array($vtprd_cart->cart_items[$k]->product_id, $vtprd_rules_set[$i]->action_group_population_info['action_group_product_incl_array'])) 
                        ||
                ( ($vtprd_cart->cart_items[$k]->parent_product_id) &&  //if this is a variation product, check for parent!
                  (in_array($vtprd_cart->cart_items[$k]->parent_product_id, $vtprd_rules_set[$i]->action_group_population_info['action_group_product_incl_array'])) ) ) {
              $vtprd_cart->cart_items[$k]->cartAuditTrail[$vtprd_rules_set[$i]->post_id]['actionPop_participation_msgs'][] = 'Product inclusion found';
               if ($vtprd_rules_set[$i]->action_group_population_info['action_group_product_and_or'] == 'and') {
                  $and_tracking++;
               }
               if ($vtprd_rules_set[$i]->action_group_population_info['action_group_and_switch_count'] == $and_tracking) {
                  return true;
...
*/
  ...
  ******************************************************** */
  ...
  //error_log( print_r('ruleset_contains_auto_add_free_product = NO, exit', true ) );
      return;
    }
    //v1.1.0.9 end
    ...
    if ($vtprd_info['current_processing_request'] != 'cart') {
   //error_log( print_r('current_processing_request] != cart, exit', true ) );
      return;
    }   
    ...
    //clear any existing save switch for this value
    //error_log( print_r(  'woo_cart_contents_with_auto_add DEL 0001 ', true ) );
    vtprd_del_transient_cart_data_by_data_type ( 'woo_cart_contents_with_auto_add'); 
    
    //$previous_auto_add_array = $this->vtprd_get_previous_auto_add_array();

    //error_log( print_r(  'before vtprd_get_transient_cart_data  0020 ', true ) );
    ...
    */
    ...
    if ($update_previous) {
        ...
        $set_previous_auto_add_array = serialize($previous_auto_add_array);
        //error_log( print_r(  'set 0007 ', true ) );
        vtprd_set_transient_cart_data ( 'previous_auto_add_array', $set_previous_auto_add_array, $cart_id );
        //v2.0.2.0 end                                 
    }
    ...
    */
    ...
    for($i=0; $i < $sizeof_rules_set; $i++) {                                                               
      ...
      /* v2.0.1.0 moved single execution above the execution of vtprd_pre_process_cart_for_autoAdds
      $this->vtprd_manage_shared_rule_tests($i); 
      ...
      */
      ...
      if ($vtprd_rules_set[$i]->actionPop == 'sameAsInPop')  {                                                  
         ...
         $vtprd_rules_set[$i]->action_group_population_info['action_group_product_incl_array'] =
            $vtprd_rules_set[$i]->buy_group_population_info['buy_group_product_incl_array']; 
         $free_product_id = $vtprd_rules_set[$i]->buy_group_population_info['buy_group_product_incl_array'][0];       //v2.0.0  FIND the FREE PRODUCT  
      } else {
          $free_product_id = $vtprd_rules_set[$i]->action_group_population_info['action_group_product_incl_array'][0];       //v2.0.0  FIND the FREE PRODUCT      
      }
         //v2.0.0 end  
      

      //v1.1.1 begin    
      $_product    =  wc_get_product( $free_product_id ); //v1.1.7 replace get_product with wc_get_product
      if ( !$_product  ) {
      ...
      if ($vtprd_rules_set[$i]->rule_deal_info[$d]['discount_auto_add_free_product'] == 'yes') {
        ...
        switch( true ) { 
          ...
          case ( $vtprd_rules_set[$i]->rule_deal_info[$d]['action_amt_type'] == 'currency' ):   
               //$_product    =  wc_get_product( $free_product_id );  //v1.1.1 moved above    //v1.1.7 replace get_product with wc_get_product
               
               //v2.0.0 begin - G solution 
               /* 
               //see if there's already a CATALOG discount
               vtprd_get_product_session_info($free_product_id);
               $vtprd_cart_item->unit_price = $vtprd_info['product_session_info']['product_discount_price'];
               */

              if ( ( isset($vtprd_info['product_session_info']['product_id']) ) &&
                   ($vtprd_info['product_session_info']['product_id'] == $free_product_id) ) {
                $all_good = true;
              } else {
                $flattened_cart_item = serialize($vtprd_cart_item);
                //v2.0.2.0 begin
                //$_SESSION['flattened_cart_item_pre_process_cart_for_autoAdds'] = $flattened_cart_item;              
                //$cart_id = $_SESSION['vtprd_unique_cart_id'];
                //error_log( print_r(  'set 0008 ', true ) );
                vtprd_set_transient_cart_data ( 'flattened_cart_item_pre_process_cart_for_autoAdds', $flattened_cart_item, $cart_id );
                //v2.0.2.0 end                

                vtprd_maybe_get_product_session_info($free_product_id);
                                 
                //v2.0.2.0 begin
                //$flattened_cart_item = $_SESSION['flattened_cart_item_pre_process_cart_for_autoAdds'];
                //error_log( print_r(  'before vtprd_get_transient_cart_data  0021', true ) );
                $flattened_cart_item = vtprd_get_transient_cart_data ( 'flattened_cart_item_pre_process_cart_for_autoAdds', $cart_id);
                vtprd_del_transient_cart_data_by_data_type ('flattened_cart_item_pre_process_cart_for_autoAdds', $cart_id);
                //v2.0.2.0 end                
              ...
              if ( (isset($vtprd_info['product_session_info']['product_id'])) &&
                ...
                $vtprd_cart_item->unit_price = vtprd_get_current_active_price($free_product_id,$_product);
              }                                                            
               //v2.0.0 end  - G solution 
               
               //v1.1.7 begin AS OF WC v 3.0.0
               if ( version_compare( WC_VERSION, '3.0.0', '<' ) ) { //check if older than version 3.0.0
                  ...
                  $unit_price  =  get_option( 'woocommerce_tax_display_cart' ) == 'excl' || $woocommerce->customer->get_is_vat_exempt() ? wc_get_price_excluding_tax($_product) : $_product->get_price();      //v1.1.7.2 changed to get_is_vat_exempt()                               
               } 
               //v1.1.7 end 
     ...
     //error_log( print_r(  'run add_to_vtprd_cart, qty= ' .$action_amt_count , true ) );         
      ...
      //error_log( print_r(  '$purchased_qty 004 = ' .$purchased_qty, true ) );
        ...
        if ( ($vtprd_rules_set[$i]->actionPop == 'sameAsInPop')  ||                                                  
            (($vtprd_rules_set[$i]->actionPop                ==   $vtprd_rules_set[$i]->inPop) &&             
             ($vtprd_rules_set[$i]->action_group_population_info['action_group_product_incl_array'] ==   //v2.0.0 
                $vtprd_rules_set[$i]->buy_group_population_info['buy_group_product_incl_array'])) ) {    //v2.0.0 
    ...
    $set_current_auto_add_array = serialize($current_auto_add_array);
    //error_log( print_r(  'set 0009 ', true ) );
    vtprd_set_transient_cart_data ( 'current_auto_add_array', $set_current_auto_add_array, $cart_id );
    //v2.0.2.0 end  
...
*/
  ...
  */
      ...
      //$previous_auto_add_array = $this->vtprd_get_previous_auto_add_array();      
      //error_log( print_r(  'before vtprd_get_transient_cart_data  0022 ', true ) );
      $get_previous_auto_add_array = vtprd_get_transient_cart_data ('previous_auto_add_array', $cart_id);
      if ($get_previous_auto_add_array)  {
          $previous_auto_add_array = unserialize($get_previous_auto_add_array);
      } else {
      ...
      //$current_auto_add_array = $this->vtprd_get_current_auto_add_array();      
      //error_log( print_r(  'before vtprd_get_transient_cart_data  0023 ', true ) );
      $get_current_auto_add_array = vtprd_get_transient_cart_data ('current_auto_add_array', $cart_id);
      if ($get_current_auto_add_array)  {
          $current_auto_add_array = unserialize($get_current_auto_add_array);
      } else {
      ...
      if ( (sizeof($previous_auto_add_array) > 0) &&
         ...
         //error_log( print_r(  'woo_cart_contents_with_auto_add DEL 0002 ', true ) );
...
//error_log( print_r(  'set $woocommerce_free_product_processed = true  001', true ) );              
  ...
  //error_log( print_r(  '$woocommerce_free_product_processed 001= ' .$woocommerce_free_product_processed, true ) );
          ...
          if ( (isset($previous_auto_add_array[$cart_product_id] )) &&
            ...
            vtprd_maybe_roll_out_prev_auto_insert_from_woo_cart($previous_auto_add_array, 'single', $cart_product_id );  //v2.0.2.0
//error_log( print_r(  'set $woocommerce_cart_updated = true  002', true ) );
  ...
  //wp_die( __('<strong>Looks like</strong>', 'vtmin'), __('VT', 'vtmin'), array('back_link' => true));  
      ...
      //error_log( print_r(  'update_parent_cart_for_autoAdds  009', true ) );
             ...
             /*
                In function vtprd_maybe_before_calculate_totals ,
                there is an execution of vtprd_process_discount in a certain situation.
             ...
             */
             //v2.0.0 end H solution
               ...
               WC()->cart->add_to_cart($current_auto_add_array_row['free_product_id'], $qty);  //v2.0.0 e solution
            }                                                                       
 ...
 //error_log( print_r(  'set $woocommerce_cart_updated = true  004', true ) );    
     ...
     //move current_auto_add_array to previous_auto_add_array
     if (sizeof($current_auto_add_array) > 0) {
       $set_current_auto_add_array = serialize($current_auto_add_array);
       //$_SESSION['previous_auto_add_array'] = serialize($current_auto_add_array);
      //error_log( print_r(  'set 0010 ', true ) );
      vtprd_set_transient_cart_data ( 'previous_auto_add_array', $set_current_auto_add_array, $cart_id );
       //v2.0.2.0 end
       ...
       vtprd_del_transient_cart_data_by_data_type ('previous_auto_add_array', $cart_id);
       $vtprd_info['previous_auto_add_array'] = array();
     }
  ...
  //error_log( print_r(  '$woocommerce_free_product_processed LAST= ' .$woocommerce_free_product_processed, true ) ); 
     ...
     $woocommerce_cart_contents = $woocommerce->cart->get_cart(); //v2.0.2.0 moved here
    ...
    //error_log( print_r(  'AT post_process auto updates, reload cart_item_key BEGIN', true ) );            
        //$woocommerce_cart_contents = $woocommerce->cart->get_cart(); v2.0.2.0 moved above
...
//wp_die( __('<strong>Looks like</strong>', 'vtmin'), __('VT Minimum Purchase not compatible - WP', 'vtmin'), array('back_link' => true));
     ...
     // 'woo_cart_contents_with_auto_add' only used when a logged-in user is picking up an old cart
     if (sizeof($current_auto_add_array) > 0) {
       $cart_contents =  serialize ($woocommerce_cart_contents);
       //error_log( print_r(  'woo_cart_contents_with_auto_add SET 0001 ', true ) );
       vtprd_set_transient_cart_data ('woo_cart_contents_with_auto_add', $cart_contents, $cart_id);              
     } else {
       //error_log( print_r(  'woo_cart_contents_with_auto_add DEL 003 ', true ) );
       vtprd_del_transient_cart_data_by_data_type ('woo_cart_contents_with_auto_add', $cart_id);
     }
     //v2.0.2.0 end
...
//$woocommerce_cart_contents = $woocommerce->cart->get_cart();
   ...
   public function vtprd_add_free_item_candidate($i) {
     ...
     //error_log( print_r(  'vtprd_add_free_item_candidate ', true ) ); 
        ...
        $cart_id = $_SESSION['vtprd_unique_cart_id'];       
        //$current_auto_add_array = $this->vtprd_get_current_auto_add_array();        
        //error_log( print_r(  'before vtprd_get_transient_cart_data  0024 ', true ) );
        $get_current_auto_add_array = vtprd_get_transient_cart_data ('current_auto_add_array',$cart_id );
        if ($get_current_auto_add_array)  {
            $current_auto_add_array = unserialize($get_current_auto_add_array);
        } else {
      ...
      if ($vtprd_rules_set[$i]->actionPop == 'sameAsInPop')  {                                                  
         ...
         $vtprd_rules_set[$i]->action_group_population_info['action_group_product_incl_array'] =
            $vtprd_rules_set[$i]->buy_group_population_info['buy_group_product_incl_array'];                    
         $free_product_id = $vtprd_rules_set[$i]->buy_group_population_info['buy_group_product_incl_array'][0]; 
      } else {
         $free_product_id = $vtprd_rules_set[$i]->action_group_population_info['action_group_product_incl_array'][0]; 
      }
        //v2.0.0 end 
    ...
    //no longer used    $vtprd_rules_set[$i]->actionPop_exploded_found_list[$last_iteration_key]->product_free_auto_insert_candidate = 'yes';
     ...
     //error_log( print_r('actionPop_exploded_group_end reset here4 = ' .$vtprd_rules_set[$i]->actionPop_exploded_group_end, true ) );           
      ...
      //$_SESSION['current_auto_add_array'] = serialize($current_auto_add_array);  
      $set_current_auto_add_array = serialize($current_auto_add_array);
      //error_log( print_r(  'set 0012 ', true ) );
      vtprd_set_transient_cart_data ( 'current_auto_add_array', $set_current_auto_add_array , $cart_id );
      //v2.0.2.0 end 
  ...
  //************
	public function vtprd_auto_add_to_vtprd_cart($free_product_id, $free_product_to_be_added_qty, $i) {
     ...
     //error_log( print_r(  'TOP OF  auto_add_to_vtprd_cart' , true ) );  
      ...
      //In obtaining any Catalog pricing, vtprd_cart_item/cart/ruleset can be overwritten, losing working addressability for CART processing.
      // EVEN THOUGH the catalog pricing is saving 'flattened_ruleset', which works everywhere else...
      // - JUST NOT HERE, requiring a SEPARATE FIX HERE!!
      ...
      //v2.0.2.0 begin
      //$_SESSION['flattened_ruleset_auto_add_to_vtprd_cart'] = $flattened_ruleset;  //flattened_ruleset2 because flattened_ruleset is used elsewhere            
      $cart_id = $_SESSION['vtprd_unique_cart_id'];
      //error_log( print_r(  'set 0013 ', true ) );
      vtprd_set_transient_cart_data ( 'flattened_ruleset_auto_add_to_vtprd_cart', $flattened_ruleset, $cart_id );
      //v2.0.2.0 end      
           
      $save_current_processing_request = $vtprd_info['current_processing_request']; 
      ...
      if ( (isset($vtprd_info['product_session_info']['product_id'])) &&
        ...
        if ( (isset($vtprd_info['product_session_info']['product_yousave_total_amt'])) &&
             ($vtprd_info['product_session_info']['product_yousave_total_amt'] > 0) ) {   
           $unit_price             =  $vtprd_info['product_session_info']['product_discount_price'];
        } else {
           $unit_price             =  $vtprd_info['product_session_info']['product_unit_price']; 
        }               
      } else {
      
        //v2.0.1.0 begin    
        $product    =  wc_get_product( $free_product_id ); //v1.1.7 replace get_product with wc_get_product
        if ( !$product  ) {
          $vtprd_rules_set[$i]->rule_status = 'freeProductNotFound'; 
          return;  
        } 
        ...
        $unit_price = vtprd_get_current_active_price($free_product_id,$product);
      }
      //v2.0.0 G Solution End   
      ...
      $vtprd_cart_item->unit_price    =  $unit_price;  //v2.0.0 G Solution
...
*/      
     ...
     //error_log( print_r(  'Adding Regular Product' , true ) );        
      ...
      //v2.0.2.0 begin
      //$flattened_ruleset = $_SESSION['flattened_ruleset_auto_add_to_vtprd_cart'];
      //error_log( print_r(  'before vtprd_get_transient_cart_data  0025 ', true ) );
      $flattened_ruleset = vtprd_get_transient_cart_data ( 'flattened_ruleset_auto_add_to_vtprd_cart', $cart_id);
      vtprd_del_transient_cart_data_by_data_type ('flattened_ruleset_auto_add_to_vtprd_cart', $cart_id);
      //v2.0.2.0 end            
      ...
      //v2.0.0 j Solution end              
       ...
       if ( version_compare( WC_VERSION, '3.0.0', '<' ) ) { //check if older than version 3.0.0
          ...
          $vtprd_cart_item->unit_price     =  get_option( 'woocommerce_tax_display_cart' ) == 'excl' || $woocommerce->customer->get_is_vat_exempt()  ? wc_get_price_excluding_tax($_product) : $_product->get_price();     //v1.1.7.2 changed to get_is_vat_exempt()   
       } 
       //v1.1.7 end 
