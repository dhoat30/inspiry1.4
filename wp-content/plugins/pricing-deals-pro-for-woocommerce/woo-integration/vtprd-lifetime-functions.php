<?php
  ...
  // ****************             
   function vtprd_rule_lifetime_validity_test($i,$shortcode = null) {  
     ...
     //v2.0.2.01 begin - fix if wp-admin or cron
     if ( (strpos($_SERVER["REQUEST_URI"],'wp-admin') !== false) ||
          (defined( 'DOING_CRON' )) ) {
        return false; //test shouldn't be done, so always send back limit not reached 
     }   
     //v2.0.2.01 end 
   ...
   function vtprd_get_purch_hist_from_session($i) {
      ...
      */
      ...
      //v2.0.2.0 begin  changed from session var
      if (!isset($_SESSION['vtprd_unique_cart_id']) ) {
        //v2.0.2.01 begin - fix if shortcode and no object...
      ...
      $hist_session_data_type = 'vtprd_rule_purch_hist_'.$rule_ID;
      $hist_session = vtprd_get_transient_cart_data($hist_session_data_type);
      
      if ($hist_session) {
        //get the data from the session var, put into rules_set
        $vtprd_rule_hist_array = unserialize($hist_session);
        ...
        $hist_session_data  = serialize($vtprd_rule_hist_array);
        vtprd_set_transient_cart_data($hist_session_data_type, $hist_session_data );
      } 
      //v2.0.2.0 end    
...
//      $rule_purchaser_rows = $wpdb->get_results("SELECT * FROM `".VTPRD_LIFETIME_LIMITS_PURCHASER_RULE."` WHERE `rule_id`  = ".$vtprd_rules_set[$i]->post_id." ",ARRAY_A);
        //v2.0.2.0  added  purchaser_user_id
    	$varsql = "SELECT  
                pr.`rule_table_id` , 
                pr.`rule_currency_total` ,
                pr.`rule_units_total` ,
                pur.`purchaser_table_id` ,
                pur.`purchaser_user_id` ,                
      ...
      if (sizeof($rule_purchaser_rows) > 0 )  {
        ...
        foreach ($rule_purchaser_rows as $rule_purchaser_row) {        
          ...
          //v2.0.2.0 begin
          //USER - compare db to current
          if ( $vtprd_setup_options['max_purch_rule_lifetime_limit_by_user'] == 'yes' )  {         
             if ( $rule_purchaser_row['purchaser_user_id'] == $current_userID ) {           
                vtprd_load_purchaser_from_db_into_rule($i, $rule_purchaser_row);
                $vtprd_rules_set[$i]->purch_hist_found_why = 'USER - compare db to current';  
    ...
    $sql_count = 0;
      ...
      */
      ...
      if ( $vtprd_setup_options['max_purch_rule_lifetime_limit_by_user'] == 'yes' )  {         
         if ( is_user_logged_in() ) {
           $current_userID = get_current_user_id(); 
           $sql_where += ' `purchaser_user_id` = ".$current_userID."  ';
           $sql_count++;
         }
      }       
  ...
  //************************     
  function vtprd_insert_purchaser_row()  {           
    global $post, $wpdb, $vtprd_setup_options, $vtprd_cart, $vtprd_rules_set, $vtprd_rule, $vtprd_info;   
      ...
      $wpdb->query("INSERT INTO `".VTPRD_LIFETIME_LIMITS_PURCHASER."` (`purchaser_table_id`,`purchaser_user_id`,`purchaser_ip_address`,`purchaser_email`,
      `billto_name`,`billto_address`, `billto_city`,`billto_state`,`billto_postcode`,`billto_country`,
      `shipto_name`,`shipto_address`, `shipto_city`,`shipto_state`,`shipto_postcode`,`shipto_country`) 
      VALUES ('{$next_id}','{$current_userID}','{$vtprd_cart->purchaser_ip_address}','{$vtprd_cart->purchaser_email}', 
      '{$vtprd_cart->billto_name}','{$vtprd_cart->billto_address}','{$vtprd_cart->billto_city}','{$vtprd_cart->billto_state}','{$vtprd_cart->billto_postcode}','{$vtprd_cart->billto_country}',
      '{$vtprd_cart->shipto_name}','{$vtprd_cart->shipto_address}','{$vtprd_cart->shipto_city}','{$vtprd_cart->shipto_state}','{$vtprd_cart->shipto_postcode}','{$vtprd_cart->shipto_country}' );");
      //
      $vtprd_cart->purchaser_table_id  = $wpdb->get_var("SELECT LAST_INSERT_ID() AS `purchaser_table_id` FROM `".VTPRD_LIFETIME_LIMITS_PURCHASER."` LIMIT 1");  
